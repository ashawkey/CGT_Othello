...
class gui_player():
    ...
    def run_screen(self):
        if 'board':
            board_leftUp = (self.width / 2 - block.size * self.game.Col / 2 + block.size / 2,
                            self.height / 2 - block.size * self.game.Row / 2 + block.size / 2)
            for i in range(self.game.Row):
                self.board.append([])
                for j in range(self.game.Col):
                    self.board[i].append(
                        block(self.tk, self.cv, board_leftUp[0] + j * block.size, board_leftUp[1] + i * block.size))
    ...
    def play_it(self, A, B, p1="computer", p2="computer"):
        ...
        while not self.player.game.finished():
            self.update_text(p1, block.second, block.first)
            A(p1)
            self.game.current = self.pos
            self.update_history()
            if self.player.game.finished():
                self.new = True
                break
            self.update_text(p2, block.first, block.second)
            B(p2)
            self.game.current = self.pos
            self.update_history()
            if self.player.game.finished():
